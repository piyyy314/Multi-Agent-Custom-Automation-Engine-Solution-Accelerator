...
class HealthCheckMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        if request.url.path == self.__healthz_path:
            ...
            # Security: Require a non-empty password to be explicitly configured
            # and use constant-time comparison (secrets.compare_digest) to prevent
            # timing side-channel attacks and unauthorized access via empty string match.
